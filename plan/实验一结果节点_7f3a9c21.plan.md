---
name: 实验一结果节点
overview: 为小核酸实验一（报告基因检测/双荧光素酶）新增三块能力：①submit_experiment_1 创建订单后，把 materialName=细胞培养板 的 4 块 96 孔板（materialId/materialName/materialCode/locationShowName）落盘为 data/细胞板名称记录/{orderId}.csv（板名尾缀 1/2/3/4，主键 materialId，不回写 LIMS）；②仿 data/gen_plate.py 新增生成器产出 data/实验一细胞培养板.xlsx，含 4 块 96 孔板 sheet（细胞培养板1..4，每孔标 空白对照/样品N-ID）作为孔位→样本身份来源；③新增 compute_experiment1_result 计算节点：读 CSV 文件 m + 孔板 xlsx，轮询 order-report 到 status==80 拿 preIntakes（每个 preIntake=一个通量，extraProperties.reportFile 两文件=renilla/firefly 的整板 384 孔 CSV，sampleMaterials[] 含本通量 4 块细胞培养板 materialId），对每块细胞培养板各自 materialId 调 material-info 拿其 96→384 映射，最终算 ratio=Renilla/Firefly、按样本聚合 ratio 均值、抑制率=1−ratio/mean(空白对照)、mean/SD（空白对照结果列暂留空）；结果表照搬实验二范式写入实验记录本（notebook_id 手填优先否则反查当前 job），同时仍输出 resultTable handle。
todos:
  - id: fixed-dir
  ...
  - id: node
    ...
    status: pending
  - id: write-notebook
    content: 照搬实验二范式把实验一结果表写入记录本——签名/goal_default/handles 加 notebook_id（输入+输出）；notebook_id 手填优先否则 _resolve_notebook_id 反查，取不到报错中止；用 nbc.text_block + nbc.build_table_node(columns→header,data→rows) 构块 append_blocks_to_notebook；不归档原始 csv【已实现：sirna_station.py compute_experiment1_result】
    status: completed
  - id: verify
...
- 列：`seq,materialId,materialName,materialCode,locationShowName`（**去掉 id 列；主键 materialId**）：
```csv
seq,materialId,materialName,materialCode,locationShowName
1,3a21c6d1-f9e1-ef91-f5de-507113076f0b,细胞培养板1,0001-00037,4-14
2,3a21c6d1-f9c7-7ccd-1246-aa235fb166f5,细胞培养板2,0001-00036,4-15
3,3a21c6d1-f99d-733f-29f7-40a73d7be122,细胞培养板3,0001-00035,4-16
4,3a21c6d1-f9e9-13f6-8201-029607be5827,细胞培养板4,0001-00038,4-17
```
...
1. **取 notebook_id（强制）**：手填优先，否则 `self._resolve_notebook_id("compute_experiment1_result")` 反查当前 job；仍取不到则 `raise ValueError(...)` 中止，不静默写空记录本。与实验二一致。
2. **构块**：实验一无图片，只一张结果表，比实验二简单。把 `result["columns"]`/`result["data"]` 转成原生表格：
...
3. **写入**：`nbc.append_blocks_to_notebook(notebook_id, blocks)`，info 日志记录 appended/total。
4. **不归档原始 csv**（用户已确认，无需 `archive_raw_files`、不调 `upload_to_oss`）。
5. **返回值**在现有 `success/order_id/resultTable/meta/confirmation_message` 基础上加 `"notebook_id": notebook_id`；`confirmation_message` 补「已写入记录本 {notebook_id}」。
...
3. 端到端跑节点，确认空白对照结果列留空、mean/SD 正确，且结果表已写入指定 notebook（手填 notebook_id 验证；不接记录本时应按强制策略报错）。
4. `make build` / import 通过；如需更新 `bioyond-sirna-station/action-index.md` 与 `actions/compute_experiment1_result.json`（补 `notebook_id` 输入/输出）。
...
- **写记录本（照搬实验二，已定）**：实验一结果表也写入实验记录本。`notebook_id` 走 `goal_default`（可手填，留空 `_resolve_notebook_id` 反查当前 job），**取不到报错中止**（强制策略，同实验二）；用 `notebook_client.build_table_node` 把 `columns/data` 转表 + 标题/空白均值说明，`append_blocks_to_notebook` 写入；**不归档原始 csv**；节点同时仍输出 `resultTable` handle，并新增 `notebook_id` 输出 handle。
