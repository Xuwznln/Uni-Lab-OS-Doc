...
todos:
  ...
  - id: compute
    ...
    status: pending
  - id: node
    content: 在 sirna_station.py 新增 @action compute_experiment1_result（输入 order_id，输出 resultTable/success），串联 读m→读xlsx→poll order-report→(每板)material-info→解析CSV→计算
    status: pending
  - id: verify
...
reportFile 文件名是时间戳（无 Renilla/Firefly 字样）。判定法：读 CSV 第 8 行 `Protocol File Path:` 是否含 `Firefly.prt` / `Renilla.prt`（样例文件该行即 `...\Biotek_Synergy_H1\Firefly.prt`）。兜底：按文件名含 renilla/firefly 判。
...
### 3.8 节点实现
仿 `compute_experiment2_result` 新增 `@action def compute_experiment1_result(self, order_id, host_prefix=r"D:\bioyond_rb\host", poll_interval_seconds=10.0, poll_timeout_seconds=3600, plate_map_path="", **kwargs)`：
- 输入 handle `order_id`（连 `wait_for_order_finish.order_id`）；输出 `resultTable`（列 板/孔位/样本/Renilla/Firefly/ratio/抑制率/抑制率均值/抑制率SD）+ `success`。
...
3. 端到端跑节点，确认空白对照结果列留空、mean/SD 正确。
4. `make build` / import 通过；如需更新 `bioyond-sirna-station/action-index.md` 与 `actions/compute_experiment1_result.json`。
