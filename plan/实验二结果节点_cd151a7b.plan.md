...
todos:
  ...
  - id: stats-split-pos
    content: "[本轮1] build_qpcr_stats_table(gen_report.py)：把「位置(目的孔 内参孔)」拆成「目的孔」「内参孔」两列；build_rows 额外暴露 pos_target/pos_ref"
    status: pending

  - id: stats-rename-cols
    content: "[本轮2] 前端统计表列名对齐图三(报告xlsx)：内参CT→内参基因、目的CT→目的基因、RQ均值→Mean、RQ标准差→SD，新增「Ave. Blank」列(仅首行显值)，去掉「样本」列(仍按样本内部分组算Mean/SD)。最终列：目的孔/内参孔/内参基因/目的基因/ΔCT/Ave. Blank/ΔΔCT/2^(-ΔΔCT)/Mean/SD"
    status: pending
