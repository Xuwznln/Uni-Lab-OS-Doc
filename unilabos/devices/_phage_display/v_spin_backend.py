...
# Unilab 驱动调试日志
_unilab_logger = logging.getLogger("unilab.driver.VSpinBackend")
...
class VSpinBackend(CentrifugeBackend):
  ...
  @action(auto_prefix=True, description="执行离心机设置与校准加载。")
  async def setup(self):
    _unilab_logger.debug("[UNILAB] VSpinBackend.setup() called")
    await self.io.setup()
  ...
  @property
  @action(auto_prefix=True, description="获取桶位 1 的校准余量。")
  def bucket_1_remainder(self) -> int:
    _unilab_logger.debug("[UNILAB] VSpinBackend.bucket_1_remainder() called")
    if self._bucket_1_remainder is None:
      raise bucket_1_not_set_error
    return self._bucket_1_remainder

  @action(auto_prefix=True, description="将当前位置设为桶位 1 并保存校准。")
  async def set_bucket_1_position_to_current(self) -> None:
    _unilab_logger.debug("[UNILAB] VSpinBackend.set_bucket_1_position_to_current() called")
  ...
  @action(auto_prefix=True, description="获取桶位 1 的位置。")
  async def get_bucket_1_position(self) -> int:
    _unilab_logger.debug("[UNILAB] VSpinBackend.get_bucket_1_position() called")
  ...
  @action(auto_prefix=True, description="停止离心机运行。")
  async def stop(self):
    _unilab_logger.debug("[UNILAB] VSpinBackend.stop() called")
    await self.configure_and_initialize()
    await self.io.stop()
  ...
  async def _get_positions_and_tachometer(self) -> _StatusPositionTachometer:
    _unilab_logger.debug("[UNILAB] VSpinBackend._get_positions_and_tachometer() called")
    """Returns 14 bytes
  ...
  @action(auto_prefix=True, description="获取当前转子位置。")
  async def get_position(self) -> int:
    _unilab_logger.debug("[UNILAB] VSpinBackend.get_position() called")
    return (await self._get_positions_and_tachometer()).current_position  # type: ignore

  @action(auto_prefix=True, description="获取当前转速。")
  async def get_tachometer(self) -> int:
    _unilab_logger.debug("[UNILAB] VSpinBackend.get_tachometer() called")
  ...
  async def get_home_position(self) -> int:
    _unilab_logger.debug("[UNILAB] VSpinBackend.get_home_position() called")
  ...
  async def _get_status(self):
    _unilab_logger.debug("[UNILAB] VSpinBackend._get_status() called")
    """
    examples:
    - 0080d0015
    - 0080f0015
  ...
  @action(auto_prefix=True, description="获取样品桶锁定状态。")
  async def get_bucket_locked(self) -> bool:
    _unilab_logger.debug("[UNILAB] VSpinBackend.get_bucket_locked() called")
    resp = await self._get_status()
    return resp[2] & 0b0001 != 0  # type: ignore

  @action(auto_prefix=True, description="获取门打开状态。")
  async def get_door_open(self) -> bool:
    _unilab_logger.debug("[UNILAB] VSpinBackend.get_door_open() called")
    resp = await self._get_status()
    return resp[2] & 0b0010 != 0  # type: ignore

  @action(auto_prefix=True, description="获取门锁定状态。")
  async def get_door_locked(self) -> bool:
    _unilab_logger.debug("[UNILAB] VSpinBackend.get_door_locked() called")
    resp = await self._get_status()
  ...
  async def _read_resp(self, timeout: float = 20) -> bytes:
    _unilab_logger.debug("[UNILAB] VSpinBackend._read_resp() called")
  ...
  async def _send_command(self, cmd: bytes, read_timeout=0.2) -> bytes:
    _unilab_logger.debug("[UNILAB] VSpinBackend._send_command() called")
  ...
  @action(auto_prefix=True, description="配置并初始化离心机。")
  async def configure_and_initialize(self):
    _unilab_logger.debug("[UNILAB] VSpinBackend.configure_and_initialize() called")
    await self.set_configuration_data()
    await self.initialize()

  @action(auto_prefix=True, description="设置设备配置数据。")
  async def set_configuration_data(self):
    _unilab_logger.debug("[UNILAB] VSpinBackend.set_configuration_data() called")
  ...
  @action(auto_prefix=True, description="初始化离心机。")
  async def initialize(self):
    _unilab_logger.debug("[UNILAB] VSpinBackend.initialize() called")
  ...
  async def open_door(self):
    _unilab_logger.debug("[UNILAB] VSpinBackend.open_door() called")
  ...
  async def close_door(self):
    _unilab_logger.debug("[UNILAB] VSpinBackend.close_door() called")
  ...
  async def lock_door(self):
    _unilab_logger.debug("[UNILAB] VSpinBackend.lock_door() called")
    if await self.get_door_open():
  ...
  async def unlock_door(self):
    _unilab_logger.debug("[UNILAB] VSpinBackend.unlock_door() called")
    if not await self.get_door_locked():
      return
    # used to be                            aa022600052d
    await self._send_command(bytes.fromhex("aa022600042c"))  # same as close door

  @action(auto_prefix=True, description="锁定样品桶。")
  async def lock_bucket(self):
    _unilab_logger.debug("[UNILAB] VSpinBackend.lock_bucket() called")
    if await self.get_bucket_locked():
      return
    await self._send_command(bytes.fromhex("aa022600072f"))

  @action(auto_prefix=True, description="解锁样品桶。")
  async def unlock_bucket(self):
    _unilab_logger.debug("[UNILAB] VSpinBackend.unlock_bucket() called")
    if not await self.get_bucket_locked():
      return
    await self._send_command(bytes.fromhex("aa022600062e"))  # same as open door

  @action(auto_prefix=True, description="将转子移动到桶位 1。")
  async def go_to_bucket1(self):
    _unilab_logger.debug("[UNILAB] VSpinBackend.go_to_bucket1() called")
    await self.go_to_position(await self.get_bucket_1_position())

  @action(auto_prefix=True, description="将转子移动到桶位 2。")
  async def go_to_bucket2(self):
    _unilab_logger.debug("[UNILAB] VSpinBackend.go_to_bucket2() called")
    await self.go_to_position(await self.get_bucket_1_position() + FULL_ROTATION // 2)

  @action(auto_prefix=True, description="将转子移动到指定位置。")
  async def go_to_position(self, position: int):
    _unilab_logger.debug("[UNILAB] VSpinBackend.go_to_position() called")
  ...
  @staticmethod
  @action(auto_prefix=True, description="将相对离心力换算为转速。")
  def g_to_rpm(g: float) -> int:
    _unilab_logger.debug("[UNILAB] VSpinBackend.g_to_rpm() called")
    # https://en.wikipedia.org/wiki/Centrifugation#Mathematical_formula
    r = 10
    rpm = int((g / (1.118 * 10**-5 * r)) ** 0.5)
    return rpm

  @action(auto_prefix=True, description="按设定离心力、时长、加速度和减速度执行离心。")
  async def spin(
    self,
    g: float = 500,
    duration: float = 60,
    ...
    _unilab_logger.debug("[UNILAB] VSpinBackend.spin() called")
    ...
    async def _reset_to_zero():
      _unilab_logger.debug("[UNILAB] VSpinBackend._reset_to_zero() called")
      await self._send_command(bytes.fromhex("aa0117021a"))
      await self._send_command(bytes.fromhex("aa01e6c800b00496000f004b00a00f050007"))
      await self._send_command(bytes.fromhex("aa0117041c"))
      await self._send_command(bytes.fromhex("aa01170119"))
  ...
  @action(auto_prefix=True, description="发送底层设备命令。")
  async def send_command(self, command: bytes) -> bytes:
    return await self._ensure_access2_backend().send_command(command)

  @action(auto_prefix=True, description="序列化设备配置或状态。")
  def serialize(self):
    return self._ensure_access2_backend().serialize()

  @action(auto_prefix=True, description="获取设备状态。")
  async def get_status(self) -> bytes:
    return await self._ensure_access2_backend().get_status()

  @action(auto_prefix=True, description="执行停放流程。")
  async def park(self):
    return await self._ensure_access2_backend().park()

  @action(auto_prefix=True, description="执行装载流程。")
  async def load(self):
    return await self._ensure_access2_backend().load()

  @action(auto_prefix=True, description="执行卸载流程。")
  async def unload(self):
    return await self._ensure_access2_backend().unload()
