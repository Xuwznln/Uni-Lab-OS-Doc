...
Original: pylabrobot/plate_reading/bmg_labtech/clario_star_backend.py
...
# Unilab 驱动调试日志
_unilab_logger = logging.getLogger("unilab.driver.CLARIOstarBackend")
...
class CLARIOstarBackend(PlateReaderBackend):
  ...
  def __init__(self, device_id: Optional[str] = None):
    print("[UNILAB] CLARIOstarBackend.__init__() called", flush=True)
    self.io = FTDI(device_id=device_id, vid=0x0403, pid=0xBB68)

  @action(auto_prefix=True, description="设置并准备酶标仪进行通信和操作。")
  async def setup(self):
    _unilab_logger.debug("[UNILAB] CLARIOstarBackend.setup() called")
    await self.io.setup()
  ...
  @action(auto_prefix=True, description="停止当前读板或设备操作。")
  async def stop(self):
    _unilab_logger.debug("[UNILAB] CLARIOstarBackend.stop() called")
    await self.io.stop()

  @action(auto_prefix=True, description="获取设备状态。")
  async def get_stat(self):
    _unilab_logger.debug("[UNILAB] CLARIOstarBackend.get_stat() called")
    stat = await self.io.poll_modem_status()
    return hex(stat)

  @action(auto_prefix=True, description="读取设备返回的响应数据。")
  async def read_resp(self, timeout=20) -> bytes:
    _unilab_logger.debug("[UNILAB] CLARIOstarBackend.read_resp() called")
  ...
  @action(auto_prefix=True, description="向酶标仪发送控制命令。")
  async def send(self, cmd: Union[bytearray, bytes], read_timeout=20):
    _unilab_logger.debug("[UNILAB] CLARIOstarBackend.send() called")
  ...
  async def _wait_for_ready_and_return(self, ret, timeout=150):
    _unilab_logger.debug("[UNILAB] CLARIOstarBackend._wait_for_ready_and_return() called")
  ...
  @action(auto_prefix=True, description="读取当前命令的执行状态。")
  async def read_command_status(self):
    _unilab_logger.debug("[UNILAB] CLARIOstarBackend.read_command_status() called")
    status = await self.send(b"\x02\x00\x09\x0c\x80\x00")
    return status

  @action(auto_prefix=True, description="初始化酶标仪并使其进入可操作状态。")
  async def initialize(self):
    _unilab_logger.debug("[UNILAB] CLARIOstarBackend.initialize() called")
  ...
  @action(auto_prefix=True, description="读取设备 EEPROM 中存储的信息。")
  async def request_eeprom_data(self):
    _unilab_logger.debug("[UNILAB] CLARIOstarBackend.request_eeprom_data() called")
  ...
  @action(auto_prefix=True, description="打开微孔板托盘。")
  async def open(self):
    _unilab_logger.debug("[UNILAB] CLARIOstarBackend.open() called")
  ...
  @action(auto_prefix=True, description="关闭微孔板托盘。")
  async def close(self, plate: Optional[Plate] = None):
    _unilab_logger.debug("[UNILAB] CLARIOstarBackend.close() called")
  ...
  async def _mp_and_focus_height_value(self):
    _unilab_logger.debug("[UNILAB] CLARIOstarBackend._mp_and_focus_height_value() called")
  ...
  async def _run_luminescence(self, focal_height: float):
    _unilab_logger.debug("[UNILAB] CLARIOstarBackend._run_luminescence() called")
  ...
  async def _run_absorbance(self, wavelength: float):
    _unilab_logger.debug("[UNILAB] CLARIOstarBackend._run_absorbance() called")
  ...
  async def _read_order_values(self):
    _unilab_logger.debug("[UNILAB] CLARIOstarBackend._read_order_values() called")
    return await self.send(b"\x02\x00\x0f\x0c\x05\x1d\x00\x00\x00\x00\x00\x00")

  async def _status_hw(self):
    _unilab_logger.debug("[UNILAB] CLARIOstarBackend._status_hw() called")
  ...
  async def _get_measurement_values(self):
    _unilab_logger.debug("[UNILAB] CLARIOstarBackend._get_measurement_values() called")
    return await self.send(b"\x02\x00\x0f\x0c\x05\x02\x00\x00\x00\x00\x00\x00")

  @action(auto_prefix=True, description="测量微孔板样品的发光信号。")
  async def read_luminescence(
    self, plate: Plate, wells: List[Well], focal_height: float = 13
  ) -> List[Dict]:
    _unilab_logger.debug("[UNILAB] CLARIOstarBackend.read_luminescence() called")
  ...
  @action(auto_prefix=True, description="测量微孔板在指定波长下的吸光度或透过率。")
  async def read_absorbance(
    ...
    _unilab_logger.debug("[UNILAB] CLARIOstarBackend.read_absorbance() called")
  ...
  @action(auto_prefix=True, description="测量微孔板样品的荧光信号。")
  async def read_fluorescence(
    ...
    _unilab_logger.debug("[UNILAB] CLARIOstarBackend.read_fluorescence() called")
