...
```python
# 默认参数，点击"否"按钮
await device.coin_cell_init()
```

或在 YAML workflow 中：
```yaml
# 使用默认值 false，不启用物料搜寻
- BatteryStation/auto-coin_cell_init: {}
```
...
- 初始化超时：120 秒（在 `coin_cell_init` 中）
