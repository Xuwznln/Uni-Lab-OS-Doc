...
class OpcUaClientWithSubscription(BaseOpcUaClient):
    ...
    def load_nodes_from_csv(self, csv_path: str) -> None:
        ...
        try:
            ...
            # 如果是相对路径，转换为绝对路径（在 workstation 与各子工站目录下查找 CSV）
            if not os.path.isabs(csv_path):
                ws_dir = os.path.dirname(os.path.abspath(__file__))
                candidate = os.path.join(ws_dir, csv_path)
                if os.path.exists(candidate):
                    csv_path = candidate
                else:
                    basename = os.path.basename(csv_path)
                    found = None
                    for sub in os.listdir(ws_dir):
                        sub_path = os.path.join(ws_dir, sub)
                        if not os.path.isdir(sub_path):
                            continue
                        alt = os.path.join(sub_path, basename)
                        if os.path.exists(alt):
                            found = alt
                            break
                    csv_path = found if found else candidate
