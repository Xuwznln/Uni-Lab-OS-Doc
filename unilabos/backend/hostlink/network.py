...
from unilabos.backend.hostlink.client import (
    HostLinkClient,
    get_hostlink_client,
    set_hostlink_client,
)
from unilabos.backend.hostlink.protocol import ActionType, PROTOCOL_VERSION
from unilabos.backend.hostlink.ros_assist import (
...
class HostNetworkService:
    ...
    def _material_search(
        ...
        return [
            item.model_dump(mode="json", exclude_none=False)
            for item in self._require_material_gateway().search_materials(name)
        ]
    ...
    def dispatch_material_sync(command: Any) -> dict[str, Any]:
        """微后端 transfer 投影下发：本进程直调设备协程，跨机经 HostLink RPC。
        ...
        result = material_sync_to_device(device_id, data)
