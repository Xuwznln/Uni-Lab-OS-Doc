"""``workflow.db`` 的 SQLModel 表记录与数据库规格。

Workflow Authority 与四库同构：本文件是 16 张表的行模型与建表 DDL 的唯一来源，
打开时按 ``schema_identity`` checksum 对齐，不再维护 user_version 迁移链。
``workflow_runs`` / ``job_runs`` 是旧审计消费方（hyper-data）依赖的只读投影视图，
随 ``frontend_event`` 的语句序列一并创建。
"""
...
WORKFLOW_DATABASE = DatabaseSpec(
    key="workflow",
    filename="workflow.db",
    role="workflow authority: definitions, tasks, jobs and frontend events",
    synchronous="NORMAL",
