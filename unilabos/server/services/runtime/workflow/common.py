...
- create_resource: device_name = 本机 host 服务设备实例名（默认 host_node，
  支持 --host_node_id 重命名，见 host_device_name()）
...
def host_device_name() -> str:
    """create_resource 执行目标：本机 host 服务设备实例名。

    实例名支持 ``--host_node_id`` 重命名（默认 host_node），在图构建时解析。
    """
