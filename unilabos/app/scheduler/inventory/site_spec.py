...
def materialize_component_sites(
    ...
    for ordinal, site in enumerate(component_sites(component)):
        ...
        name = str(site.get("label", index)).strip()
        if not name:
            raise ValueError(f"config_info Site {ordinal} label is required")
        ...
        connection.execute(
            ...
            (
                ...
                max(0.0, _number(size.get("depth"))),
                max(0.0, _number(size.get("height"))),
                max(0.0, _number(size.get("width"))),
            ),
        )
