"""工作流执行历史：独立 SQLite 持久化（第三个库）。

三库分立（读写频率与生命周期不同，互不阻塞）：

- ``inventory.db``       物料/仓储（事务重、账本语义）
- ``device_state.db``    设备遥测（高频 upsert）
- ``workflow_history.db``本模块：工作流/job 执行历史（低频 append，审计与回放）
...
写入方是 EdgeScheduler（提交/终态/每个 job 完结时同步写，WAL 小事务）；
进程重启时 ``mark_interrupted()`` 把上一世代残留的非终态 run 标记为
``interrupted``（内存调度状态不跨进程恢复，历史如实记录中断）。
"""
...
class WorkflowHistoryStore:
    """工作流/job 执行历史（独立 SQLite WAL）。"""

    def __init__(self, db_path: str = ":memory:", max_runs: int = 2000):
