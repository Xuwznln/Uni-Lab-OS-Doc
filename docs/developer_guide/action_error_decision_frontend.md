...
在 `feat/edge-networking-and-scheduler` 中，这些接口直接复用 scheduler 的四通道
MonitorBus 和 `/api/v1` Router，不额外创建第二套 Web EventBus 或重复路由。
...
| Host 微后端 | scheduler backend 适配 Host pending，为本地前端提供快照、SSE 和决策提交接口 |
...
MonitorBus 同时支持 `material/device/action/scheduler` 四个通道；异常决策前端使用
`channels=action` 过滤。当前与动作异常相关的 action 事件类型：
